...
def generate_quantified_self_csv(
    ...
    df['Acute-to-Chronic Workload Ratio - ACWR (ratio)'] = df['ATL_7d'] / df['Chronic Training Load - CTL - 28d Span / 13.5d Half-Life EWMA (load)']
    ...
    if 'Overnight Resting HR (bpm)' in df.columns:
        ...
        df['Resting HR Z-Score - 3d Span / 1d Half-Life EWMA (SD)'] = daily_rhr_z.ewm(span=3, adjust=False).mean()
